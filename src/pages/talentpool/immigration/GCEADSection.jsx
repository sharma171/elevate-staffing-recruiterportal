...
function GCEADCard({ ead, isCurrent, isEditing = false, onChange, onDelete }) {
      ...
      <Card className={`p-4 ${isCurrent ? "border-primary/50 bg-primary/5" : "border-muted bg-muted/30"}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <EditField
            ...
            onChange={(v) => handleFieldChange("card_valid_from", v)}
            type="date"
          />
          <EditField
            label="Expiry Date"
...
export function GCEADSection({
  ...
  const handleDataExtracted = (data) => {
    if (onEADDataExtracted) onEADDataExtracted(data);
  };
  const handleRawDataExtracted = async (rawData, fileInfo) => {
    if (onSaveEAD) {
      const dataWithCategory = { ...rawData, ead_category: rawData.ead_category || "C09" };
      return onSaveEAD(dataWithCategory, fileInfo);
    }
  };
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="font-medium text-gray-900">GC EAD Card</div>
          {eadRecords.length > 0 && (
            <Badge variant="secondary" className="text-xs">
              {eadRecords.length}
            </Badge>
      ...
      {hasNoEADs ? (
        <div className="border border-dashed rounded-lg p-8 text-center text-muted-foreground">
          ...
          <p className="font-medium mb-1">No GC EAD Card Data</p>
