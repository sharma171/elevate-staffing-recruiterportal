...
function AddModal({ show, setshow, defaultDocs = [], update }) {
  ...
  const handleSave = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      // Update current file as uploading
      setUploadFiles((curr) =>
        curr.map((item, idx) => (idx === i ? { ...item, status: "uploading", percent: 0 } : item)),
      );

      // Simulate running progress bar
      ...
      try {
        const res = await api.documentsUpload(payload);
